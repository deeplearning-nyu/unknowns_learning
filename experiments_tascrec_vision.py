import tascrec_vision
from tascrec_vision import get_resnet18
from tascrec_vision import cross_reclassification
from tascrec_vision import rectify_model
from tascrec_vision import evaluate_model
from tascrec_vision import get_augmented_data
...
model = tascrec_vision.get_resnet18(**config)

data_dict = tascrec_vision.get_augmented_data(sample_rate, **config)

uus_mined = tascrec_vision.cross_reclassification(model, data_dict['aug_train'],
...
for i in range(3):
    ...
    final_weights = tascrec_vision.rectify_model(model, 
                                                   data_dict['aug_train'],
                                                   uus_mined, 
                                                   rebuild=1, num_epochs=epochs, **config)
    print('Rectify completed after {}s'.format(int(time.time() - begin_time)))

    accuracy, auroc, aupr, fpr_95 = tascrec_vision.evaluate_model(model, 
                                                                    final_weights, 
                                                                    data_dict['eval'],
                                                                    **config)
